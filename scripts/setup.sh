...
# Dev environment setup for the yandex_smarthome provider.
set -euo pipefail
...
if [ -n "$WORKSPACE" ]; then
  ...
  LINK="$WORKSPACE/ma-server/music_assistant/providers/yandex_smarthome"
  TARGET="$(python3 -c "import os; print(os.path.relpath('$REPO_ROOT/provider/', '$(dirname "$LINK")'))")"
  rm -f "$LINK"
  ln -s "$TARGET" "$LINK"
  echo "Symlink: yandex_smarthome → $TARGET"
  ...
  echo "Provider yandex_smarthome linked to workspace. Activate: source $WORKSPACE/.venv/bin/activate"
else
